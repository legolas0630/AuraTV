...
export default function RegisterPage() {
  ...
  const content = {
    en: {
      ...
      subtitle: 'Sign up for free instantly. Choose a registration method below to configure your custom streaming tunnel profile.',
      ...
      signIn: 'Sign In here',
      errorMismatch: 'Passwords do not match. Please verify your credentials.'
    },
    es: {
      title: 'Crear tu Cuenta',
      subtitle: 'Regístrate gratis al instante. Selecciona un método a continuación para configurar tu perfil de transmisión personalizado.',
      ...
      errorMismatch: 'Las contraseñas no coinciden. Por favor verifica tus credenciales.'
    }
  };

  const t = content[lang as 'en' | 'es'] || content.en;

  // Handles Email & Password Signup Flow with Validation
  ...
  // Handles Native Cloud OAuth Authorization Handshakes (Google, Apple, Facebook)
  const handleSocialRegister = async (provider: 'google' | 'apple' | 'facebook') => {
    ...
    try {
      const { error } = await supabase.auth.signInWithOAuth({
        provider,
        options: {
          // Point back to your custom domain or local instance callback router
    ...
    <div className="min-h-[85vh] bg-[#f4f4f7] text-[#0a0a0c] dark:bg-[#060608] dark:text-[#f4f4f7] flex items-center justify-center p-6 transition-colors duration-200">
      <div className="w-full max-w-md bg-white dark:bg-[#0c0c10] border border-black/10 dark:border-white/5 rounded-3xl p-8 shadow-xl space-y-6">
        ...
        <form onSubmit={handleRegister} className="space-y-4">
          ...
          <div className="space-y-1">
            <label className="text-[10px] font-black tracking-wider text-gray-400 uppercase">{t.confirmPassLabel}</label>
            <input 
          ...
          >
            {loading ? 'DEPLOYING PROFILE ENVIRONMENT...' : t.cta}
          </button>
        </form>
        ...
        {/* Premium Frosted Glass Social OAuth Button Grid */}
        <div className="grid grid-cols-3 gap-2.5">
          
          {/* Google */}
          <button
            type="button" onClick={() => handleSocialRegister('google')}
          ...
          {/* Apple */}
          <button
            type="button" onClick={() => handleSocialRegister('apple')}
            className="flex items-center justify-center py-3 rounded-xl bg-black/5 dark:bg-white/[0.02] border border-black/5 dark:border-white/10 hover:bg-black/10 dark:hover:bg-white/5 font-bold text-xs transition cursor-pointer"
          >
          ...
          {/* Facebook */}
          <button
            type="button" onClick={() => handleSocialRegister('facebook')}
            className="flex items-center justify-center py-3 rounded-xl bg-black/5 dark:bg-white/[0.02] border border-black/5 dark:border-white/10 hover:bg-black/10 dark:hover:bg-white/5 font-bold text-xs transition cursor-pointer"
          >
