...
export default function PricingPage() {
  ...
  const pricingText = {
    en: {
      ...
      plans: [
        { name: '24-Hour Trial', desc: 'Test our entire stream ecosystem with absolutely zero financial commitments.', type: 'trial', billing: 'One-time access' },
        { name: 'Monthly Premium', desc: 'Full uninterrupted access to the complete global protocol suite.', type: 'monthly', billing: 'Billed monthly' }
      ]
    },
    es: {
      ...
      plans: [
        { name: 'Prueba de 24 Horas', desc: 'Pruebe todo nuestro ecosistema de transmisión sin ningún compromiso financiero.', type: 'trial', billing: 'Acceso por única vez' },
        { name: 'Premium Mensual', desc: 'Acceso completo e ininterrumpido a la suite completa del protocolo global.', type: 'monthly', billing: 'Facturado mensualmente' }
      ]
    }
  };
            ...
            return (
              <div 
                ...
                className={`relative bg-white dark:bg-white/[0.02] border rounded-3xl p-8 shadow-sm flex flex-col justify-between transition-all duration-200 transform hover:scale-[1.01] ${
                  isMonthly ? 'border-violet-500 ring-2 ring-violet-500/20' : 'border-black/5 dark:border-white/5'
                }`}
              >
                {isMonthly && (
